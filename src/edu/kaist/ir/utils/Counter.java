...
public class Counter<E> implements Serializable {
	...
	public String toString() {
		return toStringSortedByValues(false);
	}
	...
	public String toStringSortedByValues(boolean ascending) {
		final StringBuilder sb = new StringBuilder("[");
		...
		for (final Entry<E, Double> element : ascending ? getEntriesSortedByIncreasingCount() : getEntriesSortedByDecreasingCount()) {
			sb.append(String.format("%s:%s", element.getKey().toString(), f.format(element.getValue())));
			if (numKeysPrinted < size() - 1)
				sb.append(" ");
			numKeysPrinted++;

			if (numKeysPrinted == 50) {
				break;
			}
		}
