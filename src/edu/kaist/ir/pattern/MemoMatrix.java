...
public abstract class MemoMatrix {
	...
	/**
	 * Create a MemoMatrix indexed from 0...|s| and 0...|t|. The strings s and t
	 * can be accessed later by sAt(i) and tAt(j).
	 */
	MemoMatrix(Sequence s, Sequence t) {
		this.s = s;
		this.t = t;
	...
	/**
	 * Compute a new element of the matrix. This should be defined in terms of
	 * calls to get(i,j).
	 */
	abstract double compute(int i, int j);

	/**
	 * Get the value at i,j, computing it only if necessary. If it has been
	 * computed before, the stored value will be re-used.
	 */
	...
	final protected String sAt(int i) {
		return s.get(i - 1);
	}

	/**
	 * Setting printNegativeValues to 'true' will invert the values printed in
	 * the matrix by toString. This is more readable the values are always <=0.
	 */
	...
	final protected String tAt(int i) {
		return t.get(i - 1);
	}
